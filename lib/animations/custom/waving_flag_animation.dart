...
class _WavingFlagState extends State<WavingFlag> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadImage() async {
    ...
    final resizedImage = await resizeImage(image, 400); // Уменьшаем изображение до нужного размера (300px по ширине)
  ...
  Future<ui.Image> resizeImage(ui.Image image, int width) async {
    ...
    final canvas = Canvas(recorder, Rect.fromPoints(Offset(0, 0), Offset(width.toDouble(), image.height.toDouble())));
    final paint = Paint();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      child: CustomPaint( // Рисуем шевелящийся флаг
      painter: FlagPainter(_image!, _controller),
      size: Size(400, 300),
      ),
      ),
      CustomPaint( // Маска сверху
...
class FlagPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double y = 0; y < image.height; y++) {
      ...
      final srcWaving = Rect.fromLTWH(staticWidth, y, image.width - staticWidth, 1);
      final dstWaving = Rect.fromLTWH(staticWidth + dx, y, image.width - staticWidth, 1);
